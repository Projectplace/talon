...
def test_get_signature_candidate():
    ...
    with patch.object(bruteforce, "SIGNATURE_MAX_LINES", 2):
        ...
        eq_(["signature1", "", "signature2"], bruteforce.get_signature_candidate(lines))
...
def test_process_marked_candidate_indexes():
    eq_([2, 13, 15], bruteforce._process_marked_candidate_indexes([2, 13, 15], "dcc"))

    eq_([15], bruteforce._process_marked_candidate_indexes([2, 13, 15], "ddc"))

    eq_([13, 15], bruteforce._process_marked_candidate_indexes([13, 15], "cc"))

    eq_([15], bruteforce._process_marked_candidate_indexes([15], "lc"))

    eq_([15], bruteforce._process_marked_candidate_indexes([13, 15], "ld"))
